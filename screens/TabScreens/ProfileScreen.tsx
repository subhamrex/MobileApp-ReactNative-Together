...
class ProfileScreen extends React.Component <Props> {
    ...
    render(){
        ...
        if(params == undefined || params == this.props.user.uid){
            return (
                <ScrollView style={{flex:1, height:screenHeight,backgroundColor: "#f5f5dc"}}>
                    
                    <SafeAreaView style={{flex:1,backgroundColor: "#f5f5dc"}}>
                        
                        <View style={{width:'100%', height:120, flexDirection:'row', justifyContent:'space-between',alignItems:'center', backgroundColor: "#f5f5dc"}}>
                            <Image source={{uri:this.props.user?.photo}} style={{width:80, height:80, borderRadius:45, margin:10}}/>
                        ...
                        <View style={{paddingHorizontal:20, width:'100%',marginBottom:20}}>
                            ...
                            <Text>{this.props.user?.quote}</Text>
                        </View>
                        <TouchableOpacity
                        style ={{justifyContent:'center',alignItems:'center'}}
                             onPress={()=>firebase.auth().signOut()}
                        >
                        <Text style={{fontSize:19,paddingHorizontal:20,marginBottom:20, fontFamily:'logo-font',fontWeight:"bold", color:'#0095f6'}}>Log Out</Text>
                        </TouchableOpacity>
                        ...
                        keyExtractor={(item:any) => JSON.stringify(item.date) }
                        style={{flex:1,}}
                        renderItem={({ item })=> 
                            <TouchableOpacity
            ...
            return (
                <ScrollView style={{flex:1,backgroundColor: "#f5f5dc"}}>
                    <View style={{width:'100%', height:120, flexDirection:'row', justifyContent:'space-between',alignItems:'center'}}>
                        <Image source={{uri:this.props.profile?.photo}} style={{width:80, height:80, borderRadius:45, margin:20}}/>
                        <View style={{flexDirection:'row', justifyContent:'space-between'}}>
                            <View style={{justifyContent:'center',alignItems:'center', margin:20}}>
                                <Text style={{fontSize:20, fontWeight:'bold'}}>
                    ...
                    <View style={{paddingHorizontal:20, width:'100%',marginBottom:20}}>
                        ...
                        <Text>{this.props.profile?.quote}</Text>
                    </View>
                    {
...
const mapDispatchToProps = (dispatch:any) => {
    return bindActionCreators({ getUser,  followUser, unFollowUser, getPost,getPosts}, dispatch)
}
